...
func ParseMulti0(
	...
	return ParseMulti(pd, ctx, fillSubparser, fillSemantics, 0, math.MaxInt32)
}
...
func ParseMulti1(
	...
	return ParseMulti(pd, ctx, fillSubparser, fillSemantics, 1, math.MaxInt32)
}
...
func ParseOptional(
	...
	return ParseMulti(pd, ctx, fillSubparser, fillSemantics, 0, 1)
}
...
func ParseAll(
	...
	orgPos := pd.Source.pos
	subresults := make([]*ParseResult, len(fillSubparsers))

	for i, subparser := range fillSubparsers {
...
func ParseAny(
	...
	orgPos := pd.Source.pos
	allFeedback := make([]*FeedbackItem, 0, len(fillSubparsers))
	lastPos := 0

	for _, subparser := range fillSubparsers {
		pd, ctx = subparser(pd, ctx)
		if pd.Result.ErrPos < 0 {
			relPos := pd.Source.pos - orgPos
			pd.Source.pos = orgPos
